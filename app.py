...
if run_clicked:
    ...
    # Keep the original detailed execution activity below the final report.
    # This shows observable workflow work (stage status/details and concrete
    # execution events) without exposing private model reasoning.
    with st.expander("Detailed workflow activity", expanded=False):
        st.markdown(
            "<div class='section-sub'>Observable actions performed by DevPilot across the development workflow.</div>",
            unsafe_allow_html=True,
        )

        labels = {key: label for key, label, _ in STAGES}
        for key, label, _ in STAGES:
            status = state.stage_state.get(key, "pending")
            detail = state.stage_detail.get(key, "")
            icon = {
                "pending": "○",
                "active": "◉",
            ...
            st.markdown(
                f"<div class='trace-card'><div class='trace-dot{' fail' if status == 'error' else ''}'></div>"
                f"<div><div class='trace-action'>{icon} {label}</div>"
                f"<div class='trace-result'>{detail}</div></div></div>",
                unsafe_allow_html=True,
            )
            ...
            for event in state.events:
                ...
                if isinstance(result, dict):
                    if result.get("changed_files"):
                        summary = "Changed: " + ", ".join(map(str, result.get("changed_files") or []))
                    elif isinstance(result.get("pytest"), dict):
                        pytest_info = result["pytest"]
                        summary = str(
                            pytest_info.get("summary")
                            or pytest_info.get("message")
                            or pytest_info.get("error")
                            or "Test execution completed"
                        )
                    elif result.get("message"):
                        summary = str(result.get("message"))
                    elif result.get("cause"):
                        summary = str(result.get("cause"))
                    else:
                        summary = "Action completed" if event.get("ok") else "Action reported a failure"
                else:
                    summary = str(result)
